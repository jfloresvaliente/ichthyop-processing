...
#          (confidence interval or standard error) for each spawning area
# URL    :  
#=============================================================================#
recruitment_area = function(dataset, a = 0.05){
  ...
  fact     <- levels(factor(dataset$ReleaseArea))
  fact_ini <- NULL
  ...
  errors <- NULL
  for(i in fact){
    facsub <- subset(dataset, dataset$ReleaseArea == i)
