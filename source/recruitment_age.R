...
#          (confidence interval or standard error) for each age minimun
# URL    : 
#=============================================================================#
recruitment_age = function(dataset, a = 0.05){
  ...
  fact     <- levels(factor(dataset$Age))
  fact_ini <- NULL
  ...
  errors <- NULL
  for(i in fact){
    facsub <- subset(dataset, dataset$Age == i)
