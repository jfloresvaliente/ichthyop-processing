#=============================================================================#
# Name   : recruitment_zone
# Author : Jorge Flores
# Date   : 
# Version:
# Aim    : Calculate the mean, the lower and upper limits of the error bars
#          (confidence interval or standard error) for each spawning bathymetry name
# URL    :
#=============================================================================#
recruitment_zone = function(dataset, a = 0.05){
  ...
  fact     <- levels(factor(dataset$Zone_name))
  fact_ini <- NULL
  ...
  errors <- NULL
  for(i in fact){
    facsub <- subset(dataset, dataset$Zone_name == i)
