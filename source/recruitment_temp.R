...
#          (confidence interval or standard error) for each lethal temp
# URL    :
#=============================================================================#
recruitment_temp = function(dataset, a = 0.05){
  ...
  fact     <- levels(factor(dataset$Temp_min))
  fact_ini <- NULL
  ...
  errors <- NULL
  for(i in fact){
    facsub <- subset(dataset, dataset$Temp_min == i)
