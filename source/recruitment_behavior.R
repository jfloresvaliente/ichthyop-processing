#=============================================================================#
# Name   : recruitment_behavior
# Author : Jorge Flores
# Date   : 
# Version:
# Aim    : Calculate the mean, the lower and upper limits of the error bars
#          (confidence interval or standard error) for each behavior
# URL    :
#=============================================================================#
recruitment_behavior = function(dataset, a = 0.05){
  ...
  fact     <- levels(factor(dataset$Coast_Behavior))
  fact_ini <- NULL
  ...
  errors <- NULL
  for(i in fact){
    facsub <- subset(dataset, dataset$Coast_Behavior == i)
