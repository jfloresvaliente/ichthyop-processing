...
dirpath <- 'G:/ICHTHYOP/peru02km/ichthyopPeru/out/'
old_path = "/run/media/jtam/JORGE_OLD/ICHTHYOP/peru02km/ichthyopPeru/cfg/"
new_path = 'G:/ICHTHYOP/peru02km/ichthyopPeru/cfg/'
...
recruitmentzone = 1
ymax = 60
dates <- read.table(paste0(new_path, 'date_init_ichthyop.csv'), header = T, sep = ';')
...
write.table(x = dat, file = paste0(dirpath, 'ichthyop_output.csv'), sep = ';', row.names = F)

year  <- tapply(as.numeric(dat$Recruitprop), list(dat$Year), mean, na.rm = T)
day   <- tapply(as.numeric(dat$Recruitprop), list(dat$Day), mean, na.rm = T)
