#=============================================================================#
# Name   : main_get_trajectories_noshelf
...
dat <- read.table(paste0(dirpath, '/results_no_shelf/ichthyop_output.csv'), header = T, sep = ';')
...
for(i in 1:12){
  ...
  for(j in 1:length(month)){
    ...
    print(ncfile)
    trajs <- get_trajectories_noshelf(ncfile     = ncfile
  ...
  Rdata <- paste0(dirpath, '/results_no_shelf/', 'trajectoriesM', i, '.Rdata')
