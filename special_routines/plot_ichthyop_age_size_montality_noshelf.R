#=============================================================================#
# Name   : plot_ichthyop_age_size_montality_noshelf
...
dat3 <- read.table(paste0(dirpath, '/results/ichthyop_output2.csv'), header = T, sep = ';')
dat3$Recruitprop <- dat3$N_constantprop

# Larval recruitment data + constant mortality data + no_shelf
dat4 <- read.table(paste0(dirpath, '/results_no_shelf/ichthyop_output2.csv'), header = T, sep = ';')
dat4$Recruitprop <- dat4$N_constantprop
...
month4 <- recruitment_month(dat4)

depth1 <- recruitment_depth(dat1)
depth2 <- recruitment_depth(dat2)
depth3 <- recruitment_depth(dat3)
depth4 <- recruitment_depth(dat4)

bathy1 <- recruitment_bathy(dat1)
bathy2 <- recruitment_bathy(dat2)
bathy3 <- recruitment_bathy(dat3)
bathy4 <- recruitment_bathy(dat4)

zone1  <- recruitment_zone(dat1)
zone2  <- recruitment_zone(dat2)
zone3  <- recruitment_zone(dat3)
zone4  <- recruitment_zone(dat4)

png(paste0(dirpath, 'plot_ichthyop_age_size_montality_noshelf.png'), height = 850, width = 1450, res = 120)
par(mfrow = c(2,2))

mycol1 <- adjustcolor(col = 'grey', alpha.f = 0.9)
mycol2 <- adjustcolor(col = 'grey', alpha.f = 0.0)
