...
# URL    : 
#=============================================================================#
dirpath       <- 'C:/Users/jflores/Documents/ICHTHYOP/10kmparent/DEB_TC5/out_simu3/results/'
...
for(i in year){
  for(j in 1:12){
    ...
    for(k in 1:(length(lat_ini)-1)){
      ...
      recruite <- mean(subset(lat_sub, lat_sub$IfRecruited == 1)$Age)
      
      # if(is.na(recruite)) recruite = 0
      
      lat_age <- c(lat_age, recruite)
    }
    hov <- recruite
