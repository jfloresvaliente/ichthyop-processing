#=============================================================================#
# Name   : get_hovmullerAgeLatitudeByYear
# Author : 
# Date   : 
# Version:
...
hovmullerRdata <- paste0(dirpath, '/hovmuller/hovmullerAgeLatitudeByYear', lat_div, 'degrees.Rdata')
...
for(year in seq(from = years[1], to = years[2], by = 1)){print(paste('Year : ', year))
  for(i in 1:12){
    for(j in t_x){
      sub_df <- subset(df, df$Year == year & df$Month == i & df$t_x == j)
