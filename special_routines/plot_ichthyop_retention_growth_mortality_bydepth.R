#=============================================================================#
# Name   : plot_ichthyop_retention_growth_mortality_bydepth
...
# PLOTS
png(paste0(dirpath, 'plot_ichthyop_retention_growth_mortality_bydepth.png'), width = 750, height = 1250, res = 120)
par(mfrow = c(3,1))

for(j in 1:3){
  dat2 <- subset(dat, dat$Depth == depths[j])
  ...
  for(i in 1:3){
    ...
    names_ind <- NULL
    for(k in seq_along(names_col)) names_ind <- c(names_ind, which(names(dat) == names_col[k]))
    dat_sub <- dat2[, names_ind]
    ...
    list_all[[i]] <- list(month)
  }
  rm(month)
  
  retention2 <- subset(retention, retention$Depth == depths[j])
  month  <- recruitment_month(retention2)
  month1 <- list_all[[1]][[1]]
  month2 <- list_all[[2]][[1]]
  month3 <- list_all[[3]][[1]]
  ...
  lines(1:12-.13, month[,1])
  points(1:12-.13, month[,1], pch = 16, cex = .6)
  arrows(1:12-.13, month[,2], 1:12-.13, month[,3], angle = 90, code = 3, length = 0.02)
  
  lines(1:12, month1[,1], lty = 5)
  points(1:12, month1[,1], pch = 8, cex = .6)
  ...
  lines(1:12+.13, month2[,1], col = 'red')
  points(1:12+.13, month2[,1], pch = 8, cex = .6, col = 'red')
  ...
  legend('bottomleft', legend = c('Age criterion','Size criterion','Size criterion + Constant mortality'), lty = c(1,5,1), col = c(1,1,2), bty = 'n', pch = c(16,8,8))
  legend('topright' , legend = paste('Release Depth', depth[j], 'm'), bty = 'n', text.font = 2, cex = 1.25)
