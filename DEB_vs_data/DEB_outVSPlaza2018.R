...
lab      <- subset(lab, lab$Autor == 'Plaza2018')
...
dat <- subset(dat, dat$t <= 350)

# PLOTS

ggname <- paste0(dirpath, 'DEB_outVSPlaza2018.png')
ggplot(data = dat)+
  geom_line(mapping = aes(x = t, y = L_w, colour = temp), size = 1, linetype = 'solid')+
  scale_color_manual(values = cols)+
  facet_wrap(facets = ~f, ncol = 3, nrow = 2)+
  labs(x = 'Time in Days [d]', y = 'Total Length [cm]', color = 'T [ºC]')+
