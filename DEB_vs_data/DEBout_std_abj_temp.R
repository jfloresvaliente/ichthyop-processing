...
# # Plot: Max growth (Lw) (solid lines) maximal functional response (f = 1)
...
# 
# # Plot: Structure (V) (solid lines) maximal functional response (f = 1)
...
#   scale_color_manual(values = cols)+
#   scale_linetype_manual(values=c('twodash', 'solid'))+
#   labs(x = 'Age [d]', y = 'Structure (V) [cm^3]', colour = 'Model Type')+
#   facet_wrap(~temp, nrow = 1)+
#   geom_text(
#     data    = dat_text,
#     mapping = aes(x = -1, y = (ylim[2]*96/100), label = label),
...
# ggsave(filename = ggname, plot = last_plot(), width = 16, height = 6)
# 
# # Plot: Reserve (E) (solid lines) maximal functional response (f = 1)
...
# Plot: delta_M (solid lines) maximal functional response (f = 1)
